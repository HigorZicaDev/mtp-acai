const CELULAR_EMPRESA = "330648564760";
...
cardapio.metodos = {
      ...
      $.each(MEU_CARRINHO, (i, e) => {
        ...
        if (e.id.includes("milk")) {
          ...
          $.each(MILK_SHAKE["coberturas"], (idCobertura, cobertura) => {
            ...
            if (
              MEU_CARRINHO[i].coberturas.some((obj) => obj.id === cobertura.id)
            ) {
          ...
          const precoAcrescimo = ACRESCIMOS["acrescimos-comum"][0].price
            .toFixed(2)
            .replace(".", ",");

          if (e.id.includes("1l")) {
            $("#p-" + e.idCarrinho).text(
              `Pode selecionar até 6 que não havera alteração no preço total, acima de 6 será cobrado (€) ${precoAcrescimo} por cada acréscimo comum adicional:`
            );
          } else {
            $("#p-" + e.idCarrinho).text(
              `Pode selecionar até 3 que não havera alteração no preço total, acima de 3 será cobrado (€) ${precoAcrescimo} por cada acréscimo comum adicional:`
            );
          }
          ...
          $.each(
            ...
            (idAcrescimoComum, acrescimoComum) => {
              ...
              $("#acrescimoComum_" + e.id + "_" + e.idCarrinho).append(
                acrecimosComuns
              );
          ...
          $.each(
            ...
            (idAcrescimoEspecial, acrescimoEspecial) => {
              let acrecimosEspeciais = cardapio.templates.acrecimosEspecial
                ...
                .replace(
                  /\${preco}/g,
                  acrescimoEspecial.price.toFixed(2).replace(".", ",")
                );

              $("#acrescimoEspecial_" + e.id + "_" + e.idCarrinho).append(
                acrecimosEspeciais
              );
    ...
    $.each(MEU_CARRINHO, (i, e) => {
      ...
      if (!e.id.includes("milk")) {
        ...
        if (e.id.includes("1l")) {
          VALOR_ITEM =
            parseFloat(e.price) +
            cardapio.metodos.calcularValorAcrescimoComum1L(e.acrescimosComuns) +
          ...
          VALOR_ITEM =
            parseFloat(e.price) +
            cardapio.metodos.calcularValorAcrescimoComum(e.acrescimosComuns) +
            cardapio.metodos.calcularValorAcrescimoEspecial(
              e.acrescimosEspeciais
            );
        }
      ...
      if (i + 1 == MEU_CARRINHO.length) {
        $("#lblValorTotal").text(
          `(€) ${VALOR_CARRINHO.toFixed(2).replace(".", ",")}`
        );
      }
    ...
    for (var i = 0; i < MEU_CARRINHO.length; i++) {
      ...
      if (
        ...
        cardapio.metodos.mensagem(
          "Selecione pelo menos um sabor de sorvete para o " + milkshake.name
        );
        return false;
      }
    }
  ...
  resumoPedido: () => {
    ...
    if (nome.length <= 3) {
      cardapio.metodos.mensagem("S'il vous plaît, remplissez le nom.");
      $("#txtNome").focus();
      return;
    }

    if (endereco.length <= 0) {
      cardapio.metodos.mensagem("S'il vous plaît, remplissez le adresse.");
      $("#txtEndereco").focus();
      return;
    }

    if (bairro.length <= 0) {
      cardapio.metodos.mensagem("S'il vous plaît, remplissez le quartier.");
      $("#txtBairro").focus();
      return;
    }

    if (cidade.length <= 0) {
      cardapio.metodos.mensagem("S'il vous plaît, remplissez la ville.");
      $("#txtCidade").focus();
      return;
    }

    if (uf == "-1") {
      cardapio.metodos.mensagem("S'il vous plaît, remplissez le Departament.");
      $("#ddlUf").focus();
      return;
    }

    if (numero.length <= 0) {
      cardapio.metodos.mensagem("S'il vous plaît, remplissez le nombre.");
      $("#txtNumero").focus();
      return;
    }
  ...
  carregarResumo: () => {
    ...
    $.each(MEU_CARRINHO, (i, e) => {
      if (!e.id.includes("milk")) {
        ...
        $.each(e.sorvetes, (indiceSorvete, sorvete) => {
          let sorvetesResumo = cardapio.templates.acrescimosResumo.replace(
            /\${nome}/g,
            sorvete.name
          );
        ...
        $.each(e.coberturas, (indiceCobertura, cobertura) => {
          let coberturaResumo = cardapio.templates.acrescimosResumo.replace(
            /\${nome}/g,
            cobertura.name
          );
    ...
    $("#cidadeEndereco").html(
      `${MEU_ENDERECO.cidade} - ${MEU_ENDERECO.uf} / ${MEU_ENDERECO.complemento}`
    );
  ...
  finalizarPedido: () => {
    if (MEU_CARRINHO.length > 0 && MEU_ENDERECO != null) {
      var texto = `Bonjour, je souhaite passer une commande!\n\n`;
      texto += `Prenom + Nom: *${MEU_NOME}*\n\n`;
      texto += `*Ma commande effectuée via le Menu Digital:*`;
      texto += `\n\n*Items commandés:*\${itens}`;
      texto += "\n\n*Adresse de livraison:*";
      texto += `\n${MEU_ENDERECO.endereco}, ${MEU_ENDERECO.numero}, ${MEU_ENDERECO.bairro}`;
      texto += `\n${MEU_ENDERECO.cidade} - ${MEU_ENDERECO.uf} / ${MEU_ENDERECO.complemento}`;
      texto += `\n\n*Total: (€) ${VALOR_CARRINHO.toFixed(2).replace(
        ".",
        ","
          ...
          $.each(acrescimosComuns, (index, acrescimo) => {
            if (e.id.includes("1l")) {
              if (index < 6) {
                ...
                const formattedPrice = `(€) ${acrescimo.price
                  .toFixed(2)
                  .replace(".", ",")}`;
                itens += `* ${acrescimo.name} ${dots} ${formattedPrice}\n`;
              }
  ...
  carregarBotaoWhatsap: () => {
    $(".botao-whatsapp").attr(
      "href",
      `https://wa.me/${CELULAR_EMPRESA}?text=Bonjour, j'ai besoin d'une commande spécifique, non disponible sur Cardapio Digital.`
    );
  },
  ...
  lojaAbertaOuFechada: () => {
    ...
    if (hora < LOJA_ABRE || hora >= LOJA_FECHA || dia == 1) {
      ...
      cardapio.metodos.mensagem(
        `Ouvert (Mardi à Dimanche) às ${LOJA_ABRE.toFixed(2).replace(
          ".",
          ":"
        )} hrs.`,
        (cor = "red"),
      ...
      cardapio.metodos.mensagem(
        "Magasin ouverte, passez votre commande !",
        (cor = "green"),
        (tempo = 6000)
      );
    }
...
cardapio.templates = {
    ...
    <div class="col-12 col-lg-3 col-md-3 col-sm-6 mb-3 wow fadeInUp">
        <div class="card card-item" id="\${id}">
            ...
            <div class="add-carrinho">
                <span class="btn-add d-flex justify-content-center align-items-center" title="Ajouter au panier" onclick="cardapio.metodos.adicionarAoCarrinho('\${id}');cardapio.metodos.animarBadgeTotal()">
    ...
    <div class="itemCarrinho" id="item-carrinho_\${id}_\${idCarrinho}">
        <div class="col-12 item-carrinho">
            ...
            <div class="dados-produtos">
                ...
                <p class="price-produto"><b  id="preco_\${id}_\${idCarrinho}">(€) \${preco}</b></p>
            </div>
            <div class="add-carrinho">
        ...
        <div class="col-12 acrescimos animated bounceInDown" id="acrescimos-\${idCarrinho}">
                ...
                <p id="p-\${idCarrinho}">Vous pouvez en sélectionner jusqu'à 3 et il n'y aura aucun changement dans le prix total, au-dessus de 3, des (€) vous seront facturés pour chaque augmentation commune supplémentaire:</p>
  ...
  itemCarrinho2: `
    <div class="itemCarrinho" id="item-carrinho_\${id}_\${idCarrinho}">
        ...
        <div class="col-12 acrescimos animated bounceInDown" id="sorvetes-\${idCarrinho}">
                <p class="title-produto"><b>Choisissez la crème glacée</b></p>
                <p>Jusqu'à 2 saveurs de glace peuvent être sélectionnées: </p>
                ...
                <p>Choisissez une saveur de sirop: </p>
  ...
  acrecimosEspecial: `
    <div class="acrescimo">
        ...
        <label for="\${id}_\${idCarrinho}" class="checkbox-custom-label">\${nome} <br>(€)\${preco}</label>
    </div>`,
